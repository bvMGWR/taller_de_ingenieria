<?php
// Configuración de la conexión para GitHub Codespaces (Ajustar credenciales de MySQL si es necesario)
$host = '127.0.0.1'; 
$db   = 'biblioteca_db';
$user = 'root'; 
$pass = '';     
...
<head>
    ...
    <title>Biblioteca de Reseñas — Aplicación</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 40px; background-color: #f4f4f9; color: #333; }
        
        /* --- ESTILOS DEL MENÚ DESPLEGABLE NAVEGABLE --- */
        .nav-container { background: white; padding: 15px 25px; border-radius: 5px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center; }
        .nav-label { font-weight: bold; color: #555; margin-right: 10px; }
        .nav-select { padding: 8px 15px; font-size: 1rem; border: 1px solid #28a745; border-radius: 4px; background-color: white; cursor: pointer; color: #28a745; font-weight: bold; }
...
<body>
    ...
    <div class="nav-container">
        <span style="font-size: 1.2rem; font-weight: bold; color: #333;">💻 Modo Aplicación Interactiva</span>
        <div>
            <label class="nav-label" for="menuNavegacion">Ir a:</label>
            <select class="nav-select" id="menuNavegacion" onchange="location = this.value;">
                <option value="index.php">📄 Ver Presentación (Inicio)</option>
                <option value="app.php" selected>💻 Ejecutar Aplicación (Calificaciones)</option>
            </select>
        </div>
    </div>
